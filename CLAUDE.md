...
### ⚠️ PIVOT IN PROGRESS: BKwire CSV is replacing PACER (2026-08-18, NOT yet confirmed)

The client is evaluating **BKwire** as a PACER replacement. Its daily export is the Form 204
...
- **Three NOT NULL columns the feed cannot fill:** `court_district` → `'BKWIRE'` sentinel,
  ...
  rows. Adding an `unknown` enum member is the real fix — deferred while BKwire is unconfirmed.
...
- **Do not spend further effort on KD-75 / KD-69 / KD-70 until BKwire is settled.**
