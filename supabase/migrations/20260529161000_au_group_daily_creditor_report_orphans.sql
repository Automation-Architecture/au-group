-- Include new company creditors even without bankruptcy_creditors / source_bankruptcy_id
-- (matches new_creditors count when the row is a non-junk company).
...
comment on function public.au_group_daily_creditor_report_rows is
  'SYS-09: non-junk company creditors created since p_since, or on bankruptcies discovered since p_since (no bankruptcy link required).';
