-- Fix SYS-09 daily sheet: creditors link via source_bankruptcy_id (bankruptcy_creditors often empty);
-- include creditors on bankruptcies discovered in window; return jsonb wrapper so n8n always gets one item.
...
comment on function public.au_group_daily_creditor_report_rows is
  'SYS-09: company creditors for daily sheet (new creditors or creditors on bankruptcies discovered since p_since). Returns {since, row_count, rows}.';
