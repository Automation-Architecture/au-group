...
- **RPC ACL:** `20260602150201_revoke_public_execute_service_role_rpcs.sql` — `REVOKE EXECUTE … FROM public` for all `service_role`-only helpers (KD-68 RSS/keyword, KD-69 config, KD-65 ZoomInfo, legacy pipeline RPCs). Inline revokes added to KD-64/66/68/69, SYS-04, RSS per-item migrations.
- **Security (2026-05-29):** `20260602150300_security_rpc_acl_and_config_hardening.sql` — CRITICAL: `au_group_upsert_document_parse_result` revoked from `anon`/`authenticated`; KD-24 RPC ACL re-assert; config table RLS deny policies; `pattern` length ≤200 + normalize input cap 500 chars. Verify: `psql … -f scripts/supabase/verify-rpc-acl.sql`.
- **Remote apply (2026-05-29, `umivttszdnsrosbqryia`):** MCP migrations `kd24_cache_key_stable_volatility`, `revoke_public_execute_service_role_rpcs` (remote-safe: no 2-arg `acquire_processing_job`; no `recompute_exposure`), `security_rpc_acl_and_config_hardening`, `revoke_anon_authenticated_au_group_rpcs`. Post-check: 0 `au_group_*` execute grants for `anon`/`authenticated`/`public`; normalize smoke `Acme Inc.` → `ACME`. `supabase db push` still blocked by MCP vs repo version drift — use MCP or `migration repair` before CLI push.
- **Volatility:** `au_group_company_lookup_cache_key` recreated as **STABLE** (was IMMUTABLE in KD-20) because it calls table-driven `au_group_normalize_company_name`. Follow-up `20260602150100_kd24_cache_key_stable_volatility.sql` for remotes that applied KD-24 before this fix. Cache keys intentionally change when `au_group_company_name_rules` are edited (existing cache rows may miss until TTL).
