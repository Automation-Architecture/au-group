...
| **OD-8** | **✅ RESOLVED (2026-06-14): discovery via the CourtListener Search API** (`pipeline/discovery.py`, built + verified live — 60 real Chapter 11 cases discovered across njb+nysb in a 14-day window). Free, uses the RECAP token, **needs no standard PACER account**. PACER Monitor API is confirmed dead; standard-PACER **PCL is a deferred authoritative add** — `intake.py` auto-selects PCL when PACER creds are configured, else CourtListener. Form 204 retrieval is RECAP-first (`pipeline/retrieval.py`). Chapter filter is server-side `q=chapter:11`; **caveat: ~13% of brand-new filings arrive chapter-blank and are missed until CourtListener enriches them** (mitigation = a no-filter sweep + re-check, deferred). _Historical framing:_ Keith has a **PACER Monitor** subscription (pacermonitor.com, a Fitch Solutions service). Their REST API offers normalized JSON from 200+ courts + programmatic document downloads + Python SDK, which would eliminate the UNVERIFIED CM/ECF scraping in the current `intake.py`. However, their API docs are gated behind account setup — capabilities (esp. Ch.11 + state + date filtering for daily discovery) cannot be verified without Keith's API trial access. **Option A — PACER Monitor API** (preferred if it supports Ch.11/state/date search + Form 204 download): wait for Keith to receive docs from `support@pacermonitor.com` and pivot `PacerClient` to their SDK. **Option B — Official PACER PCL REST** (`pcl.uscourts.gov`): what `intake.py` is currently built against; full API spec in `docs/architecture/pacer-pcl-api-reference.md`; document download is UNVERIFIED CM/ECF scraping. **Decision trigger:** Keith receives PACER Monitor API docs → evaluate against the two AC items (case-search filtering + Form 204 download). If PACER Monitor can't do both → switch to Option B (PCL + direct PACER creds). | A: PACER Monitor API · B: PACER PCL REST | Keith (API docs) + Operator | Blocks WP-05 finalization |
...
| PACER data source undecided (OD-8) | Medium | `intake.py` built against PCL REST (Option B); `PacerClient` class is the only change if PACER Monitor (Option A) wins — all downstream logic (S3, upsert, enqueue) is identical. Await Keith's PACER Monitor API docs; if not sufficient, use PCL creds directly. |
